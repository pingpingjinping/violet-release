...
Uint8List encodeBackup(Map<String, dynamic> payload) {
  ...
  if (raw.length > maxBackupRaw)
    throw const FormatException('백업 원본이 128MiB를 초과합니다.');
  final bytes = Uint8List.fromList(gzip.encode(raw));
  if (bytes.length > maxBackupCompressed)
    throw const FormatException('압축 백업이 16MiB를 초과합니다.');
  return bytes;
}
...
Map<String, dynamic> decodeBackup(Uint8List bytes) {
  ...
  for (var offset = 0; offset < bytes.length; offset += 4096) {
    converter.add(
      bytes.sublist(offset, (offset + 4096).clamp(0, bytes.length).toInt()),
    );
  }
  converter.close();
  final value = jsonDecode(utf8.decode(output.builder.takeBytes()));
  if (value is! Map<String, dynamic>)
    throw const FormatException('잘못된 백업 파일입니다.');
...
void validateBackup(Map<String, dynamic> payload) {
  ...
  if (payload['schema'] != 1 ||
      user is! String ||
      !RegExp(r'^[\x21-\x7e]{1,256}$').hasMatch(user) ||
  ...
  for (final required in [
    ...
    if (!tables.containsKey(required))
      throw const FormatException('필수 백업 항목이 없습니다.');
  }
  var total = 0;
  for (final entry in tables.entries) {
    final columns = backupColumns[entry.key];
    final rows = entry.value;
    if (columns == null || rows is! List)
      throw const FormatException('잘못된 백업 테이블입니다.');
    total += rows.length;
    ...
    for (final row in rows) {
      ...
      if (columns.contains('Id')) {
        final id = row['Id'];
        if (id is! int || id < 1 || !ids.add(id))
          throw const FormatException('잘못된 기록 ID입니다.');
      }
      if (entry.key == 'SharedActivity') {
        if (!['read', 'download'].contains(row['Kind']) ||
            row['Device'] is! String ||
            row['Article'] is! String ||
            !RegExp(r'^\d{1,20}$').hasMatch(row['Article'] as String) ||
            !['app', 'web'].contains(row['Origin']) ||
            row['Timestamp'] is! int ||
            row['Page'] is! int ||
            row['Type'] is! int ||
            (row['Timestamp'] as int) <= 0 ||
            (row['Page'] as int) < 0 ||
            (row['Page'] as int) > 1000000 ||
            ![0, 1].contains(row['Type'])) {
          throw const FormatException('잘못된 공유 기록입니다.');
        }
      }
    }
  }
  final groups = (tables['BookmarkGroup'] as List)
      .map((r) => (r as Map)['Id'])
      .toSet();
  ...
  for (final name in ['BookmarkArticle', 'BookmarkArtist', 'BookmarkUser']) {
    for (final row in tables[name] as List? ?? []) {
      if (!groups.contains((row as Map)['GroupId']))
        throw const FormatException('북마크 폴더 연결이 잘못되었습니다.');
    }
  }
}
