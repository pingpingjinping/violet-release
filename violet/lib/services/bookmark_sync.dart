...
class BookmarkSync {
  ...
  static Future<int?> sync({bool force = false}) => _lock.synchronized(() async {
    ...
    final endpoint = '${baseUrl.replaceFirst(RegExp(r"/+$"), "")}/api/bookmark-sync';
    ...
    final db = manager.db!;
    await db.execute('CREATE TABLE IF NOT EXISTS BookmarkSyncMeta '
        '(Id INTEGER PRIMARY KEY, State TEXT NOT NULL)');

    final state = await db.transaction<Map<String, dynamic>?>((txn) async {
      final rows = await txn.query('BookmarkSyncMeta', where: 'Id=?', whereArgs: [1]);
      final saved = rows.isEmpty
          ? <String, dynamic>{'revision': 0, 'shadow': <String>[], 'lastSuccess': 0}
          : Map<String, dynamic>.from(jsonDecode(rows.first['State'] as String) as Map);
      final now = DateTime.now().millisecondsSinceEpoch;
      if (!force && now - (saved['lastSuccess'] as int) < days * 86400000) return null;
      ...
      await txn.rawInsert('INSERT OR REPLACE INTO BookmarkSyncMeta VALUES (?, ?)', [1, jsonEncode(saved)]);
      return saved;
    });
    ...
    }, body: jsonEncode(pending['payload'])).timeout(const Duration(seconds: 60));
    if (response.statusCode != 200) throw Exception('Sync HTTP ${response.statusCode}');
    final data = jsonDecode(response.body) as Map<String, dynamic>;
    final revision = data['revision'];
    final articles = data['articles'];
    if (revision is! int || revision < (state['revision'] as int) ||
        articles is! List || articles.length > 100000 ||
    ...
    await db.transaction((txn) async {
      ...
      final group = groups.firstWhere((g) => g['Name'] == 'violet_default', orElse: () => groups.first);
      final groupId = group['Id'];
      final batch = txn.batch();
      for (final row in rows) {
        final id = row['Article'].toString();
        if (_numeric.hasMatch(id) && !wanted.contains(id)) {
          batch.delete('BookmarkArticle', where: 'Id=?', whereArgs: [row['Id']]);
        }
      }
      ...
      await txn.rawInsert('INSERT OR REPLACE INTO BookmarkSyncMeta VALUES (?, ?)', [1, jsonEncode({
        'revision': revision, 'shadow': remote.toList(),
