...
class PiBackup {
  static Future<(String, Map<String, String>)> _connection() async {
    ...
    if (base.isEmpty || token.isEmpty) throw const FormatException('앱·웹 기록 동기화 설정에 개인 서버 주소와 토큰을 먼저 저장해 주세요.');
  ...
  static Future<Map<String, dynamic>> snapshot() async {
    ...
    final tables = await manager.db!.transaction((txn) async {
      final names = (await txn.rawQuery("SELECT name FROM sqlite_master WHERE type='table'")).map((r) => r['name']).toSet();
      final result = <String, List<Map<String, dynamic>>>{};
      for (final name in backupColumns.keys) {
        if (names.contains(name)) result[name] = (await txn.query(name)).map((r) => Map<String, dynamic>.from(r)).toList();
      }
      final device = prefs.getString('activity_sync_device') ?? 'backup-$userId';
      if (names.contains('DownloadItem')) {
        final rows = await txn.query('DownloadItem', columns: ['URL', 'DateTime'], where: 'State=?', whereArgs: [0]);
        final shared = result['SharedActivity'] ??= [];
        for (final row in rows) {
          final article = row['URL']?.toString() ?? '';
          final timestamp = ActivitySync.timeOf(row['DateTime']);
          if (!RegExp(r'^\d{1,20}$').hasMatch(article) || timestamp <= 0) continue;
          shared.add({'Kind': 'download', 'Device': device, 'Article': article, 'Origin': 'app', 'Timestamp': timestamp, 'Page': 0, 'Type': 0});
        }
      }
      return result;
    });
    return {'schema': 1, 'userAppId': userId, 'createdAt': DateTime.now().toUtc().toIso8601String(), 'tables': tables};
  }

  static void _check(http.Response response) {
    if (response.statusCode == 200) return;
    if (response.statusCode == 401) throw const FormatException('개인 서버 토큰이 맞지 않습니다.');
    if ([404, 405].contains(response.statusCode)) throw const FormatException('서버에 전체 백업 기능이 없습니다. Pi 서버 업데이트가 필요합니다.');
    if ([400, 413].contains(response.statusCode)) throw const FormatException('서버가 백업 파일을 거부했습니다. 형식과 용량을 확인해 주세요.');
  ...
  static Future<Map<String, dynamic>> create() async {
    ...
    try {
      final response = await client.post(Uri.parse(ServerConfig.endpoint(base, 'api/backups')),
        headers: {...headers, 'Content-Type': 'application/gzip', 'X-Violet-User-App-Id': payload['userAppId'] as String}, body: data).timeout(const Duration(minutes: 2));
      _check(response);
  ...
  static Future<Map<String, dynamic>> _download(Map<String, dynamic> meta) async {
    final (base, headers) = await _connection();
    final id = meta['id'] as String;
    if (!RegExp(r'^[0-9a-f-]{36}$').hasMatch(id)) throw const FormatException('잘못된 백업 ID입니다.');
    final client = http.Client();
    try {
      final request = http.Request('GET', Uri.parse(ServerConfig.endpoint(base, 'api/backups/$id')))..headers.addAll(headers);
      final response = await client.send(request).timeout(const Duration(seconds: 15));
      if (response.statusCode != 200) { _check(http.Response('', response.statusCode)); }
      final bytes = BytesBuilder(copy: false);
      var length = 0;
      await for (final chunk in response.stream.timeout(const Duration(seconds: 60))) {
        length += chunk.length;
        if (length > maxBackupCompressed) throw const FormatException('압축 백업이 너무 큽니다.');
        bytes.add(chunk);
      }
  ...
  static Future<String> restore(Map<String, dynamic> meta) => BookmarkSync.operationLock.synchronized(() async {
    ...
    final rollback = File('${directory.path}/before-restore-${DateTime.now().microsecondsSinceEpoch}.json.gz');
...
Future<void> restoreBackupTables(Database db, Map<String, dynamic> payload) async {
  validateBackup(payload);
  final tables = payload['tables'] as Map;
  await db.transaction((txn) async {
    final existing = (await txn.rawQuery("SELECT name FROM sqlite_master WHERE type='table'")).map((r) => r['name']).toSet();
    for (final name in tables.keys) {
      if (!existing.contains(name)) throw FormatException('기기에 필요한 백업 테이블이 없습니다: $name');
    }
    ...
    for (final name in backupColumns.keys) {
      for (final row in tables[name] as List? ?? []) {
        batch.insert(name, Map<String, Object?>.from(row as Map), conflictAlgorithm: ConflictAlgorithm.replace);
      }
    }
