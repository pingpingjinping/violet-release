...
class ActivitySync {
  ...
  static const _schema = 'CREATE TABLE IF NOT EXISTS SharedActivity ('
  ...
  static Future<void> _load() async {
    ...
    records.value = (await manager.db!.query('SharedActivity', orderBy: 'Timestamp DESC'))
  ...
  static List<String> downloadedOrigins(String article) => records.value
      ...
      .map((r) => r['Origin'] as String).toSet().toList();

  static int timeOf(Object? value) => DateTime.tryParse(value?.toString() ?? '')
      ?.toUtc().millisecondsSinceEpoch ?? 0;

  static Future<void> sync({bool force = false}) => _lock.synchronized(() async {
    ...
    if (!force && DateTime.now().millisecondsSinceEpoch - last < days * 86400000) return;
    await load();
    final manager = await CommonUserDatabase.getInstance();
    final db = manager.db!;
    ...
    if (names.contains('ArticleReadLog')) {
      for (final row in await db.query('ArticleReadLog')) {
        ...
        if (!read.containsKey(article) || (read[article]!['timestamp'] as int) < timestamp) {
          final nativePage = (row['LastPage'] as int?) ?? 1;
          read[article] = {'article': article, 'timestamp': timestamp,
            'page': nativePage > 0 ? nativePage - 1 : 0, 'type': row['Type'] == 1 ? 1 : 0};
        }
      }
    }
    final downloads = <String, Map<String, dynamic>>{};
    if (names.contains('DownloadItem')) {
      for (final row in await db.query('DownloadItem', where: 'State=?', whereArgs: [0])) {
        final article = row['URL']?.toString() ?? '';
        final timestamp = timeOf(row['DateTime']);
        if (!_numeric.hasMatch(article) || timestamp <= 0) continue;
        if (!downloads.containsKey(article) || (downloads[article]!['timestamp'] as int) < timestamp) {
          downloads[article] = {'article': article, 'timestamp': timestamp};
        }
      }
    }
    final endpoint = '${base.replaceFirst(RegExp(r"/+$"), "")}/api/activity-sync';
    final response = await http.post(Uri.parse(endpoint), headers: {
      'Content-Type': 'application/json', 'X-Violet-Sync-Token': token,
    }, body: jsonEncode({'device': device, 'origin': 'app',
      'read': read.values.toList(), 'download': downloads.values.toList()}))
        .timeout(const Duration(seconds: 60));
    if (response.statusCode != 200) throw Exception('Activity sync failed');
    final data = jsonDecode(response.body) as Map<String, dynamic>;
    if (data['version'] != 1 || data['records'] is! List || (data['records'] as List).length > 100000) {
    ...
    for (final value in data['records'] as List) {
      ...
      if (!['read', 'download'].contains(r['kind']) || !['app', 'web'].contains(r['origin']) ||
          r['device'] is! String || !RegExp(r'^[0-9a-f-]{36}$').hasMatch(r['device'] as String) ||
          r['article'] is! String || !_numeric.hasMatch(r['article'] as String) ||
          r['timestamp'] is! int || (r['timestamp'] as int) <= 0 || (r['timestamp'] as int) > 8640000000000000 ||
          r['page'] is! int || (r['page'] as int) < 0 || (r['page'] as int) > 1000000 || ![0, 1].contains(r['type'])) {
        throw Exception('Invalid activity record');
      }
      incoming.add({'Kind': r['kind'], 'Device': r['device'], 'Article': r['article'],
        'Origin': r['origin'], 'Timestamp': r['timestamp'], 'Page': r['page'], 'Type': r['type']});
    }
    await db.transaction((txn) async {
      await txn.delete('SharedActivity');
      final batch = txn.batch();
      for (final row in incoming) { batch.insert('SharedActivity', row); }
      await batch.commit(noResult: true);
    });
    records.value = incoming;
    await prefs.setInt('activity_sync_last_success', DateTime.now().millisecondsSinceEpoch);
  });
}
