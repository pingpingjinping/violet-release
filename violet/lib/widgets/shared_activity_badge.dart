...
class _SharedActivityBadgeState extends State<SharedActivityBadge> {
  ...
  @override
  Widget build(BuildContext context) => ValueListenableBuilder<List<Map<String, dynamic>>>(
    valueListenable: ActivitySync.records,
    builder: (context, rows, _) {
      final origins = ActivitySync.downloadedOrigins(widget.article);
      final read = rows.where((r) => r['Kind'] == 'read' && r['Article'] == widget.article).firstOrNull;
      final labels = <String>[
      ...
      return IgnorePointer(child: Container(
        ...
        child: Text(labels.join(' / '), maxLines: 2, overflow: TextOverflow.ellipsis,
