...
class User {
  ...
  Future<List<ArticleReadLog>> getUserLog() async {
    ...
    for (final log in cachedReadLog!) {
      ...
      if ((localLatest[log.articleId()] ?? 0) < time) localLatest[log.articleId()] = time;
    }
    final sharedLatest = <String, Map<String, dynamic>>{};
    for (final r in ActivitySync.records.value.where((r) => r['Kind'] == 'read')) {
    ...
    final combined = <ArticleReadLog>[
      ...cachedReadLog!,
      for (final r in sharedLatest.values) ArticleReadLog(result: {
        'Id': remoteId--, 'Article': r['Article'], 'Type': r['Type'],
        'DateTimeStart': DateTime.fromMillisecondsSinceEpoch(r['Timestamp'] as int, isUtc: true).toIso8601String(),
        'DateTimeEnd': DateTime.fromMillisecondsSinceEpoch(r['Timestamp'] as int, isUtc: true).toIso8601String(),
        'LastPage': (r['Page'] as int) + 1,
      }),
    ];
    combined.sort((a, b) => ActivitySync.timeOf(b.datetimeEnd() ?? b.datetimeStart())
