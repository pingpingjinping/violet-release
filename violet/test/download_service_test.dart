...
void main() {
  ...
  test('Work starts without a download page, stays serial and holds the screen until idle', () async {
    final wake = <bool>[];
    final first = Completer<void>();
    final started = Completer<void>();
    final order = <int>[];
    final queue = DownloadWorkQueue(keepAwake: (value) async { wake.add(value); });
    final a = queue.submit(1, () async {
      order.add(1);
      started.complete();
      await first.future;
    });
    final duplicate = queue.submit(1, () async { fail('duplicate gallery ran'); });
    final b = queue.submit(2, () async { order.add(2); });
  ...
  test('A failed gallery releases the screen and does not block the next gallery', () async {
    final wake = <bool>[];
    var secondRan = false;
    final queue = DownloadWorkQueue(keepAwake: (value) async { wake.add(value); });
    final failed = queue.submit(1, () async { throw StateError('failed'); });
    final checked = expectLater(failed, throwsStateError);
    final next = queue.submit(2, () async { secondRan = true; });
    await checked;
    await next;
    await Future<void>.delayed(Duration.zero);
  ...
  test('A request arriving while the screen lock releases is not lost', () async {
    final releasing = Completer<void>();
    final release = Completer<void>();
    var firstRelease = true, secondRan = false;
    final wake = <bool>[];
    final queue = DownloadWorkQueue(keepAwake: (value) async {
      wake.add(value);
    ...
    final second = queue.submit(2, () async { secondRan = true; });
    release.complete();
    await second;
    await Future<void>.delayed(Duration.zero);
    expect(secondRan, true);
  ...
  test('Platform screen-lock errors do not prevent downloading', () async {
    var ran = false;
    final queue = DownloadWorkQueue(keepAwake: (_) async { throw StateError('platform'); });
    await queue.submit(1, () async { ran = true; });
    await Future<void>.delayed(Duration.zero);
    expect(ran, true);
    expect(queue.totalCount, 0);
    queue.dispose();
  ...
  test('Real downloader cancels queued files and retry byte counts start from zero', () async {
    ...
    try {
      ...
      final first = DownloadTask(url: 'http://127.0.0.1:${server.port}/slow',
        downloadPath: '${root.path}/first', headers: {});
      first.completeCallback = () { completed.complete(); };
      final cancelled = DownloadTask(url: 'http://127.0.0.1:${server.port}/cancelled',
        downloadPath: '${root.path}/cancelled', headers: {});
      downloader.appendTasks([first, cancelled]);
      await firstRequest.future;
      downloader.cancel(cancelled.taskId);
      expect(downloader.getStatus(cancelled.taskId).state, DownloadTaskState.cancel);
      release.complete();
      ...
      var received = 0.0;
      final retry = DownloadTask(url: 'http://127.0.0.1:${server.port}/retry',
        downloadPath: '${root.path}/retry', headers: {});
      retry.accDownloadSize = 8192;
      retry.isSizeEnsued = true;
      retry.completeCallback = () { retryDone.complete(); };
      retry.downloadCallback = (bytes) { received += bytes; };
